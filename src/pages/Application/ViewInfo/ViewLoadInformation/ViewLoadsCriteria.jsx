...
const ViewLoadsCriteria = ({
  ...
  const browse = ({ HideEmptyColumns }) => {
    addHistoryLink({
      title: t('sts.window.browse.loads'),
      path: `${window.origin}/${ROUTER_PATH.browseLoads}/${loadedInfo.ID}?hideEmptyCols=${
        HideEmptyColumns || ''
      }`,
  ...
  const matchNumber = async (prefix) => {
    ...
    try {
      const { Entries } = await loadRefJobs({ prefix });
  ...
  const RenderMultiSelect = ({
    ...
    return (
      <CustomInputMultiselect
        ...
        customSuggestionsStart={
          DEFAULT_SELECTION_FOR.includes(ID) ? [{ Name: '{}', ID: '{}' }] : []
        }
  ...
  return (
    <div className="h-full flex flex-column fadein">
      <Dialog visible={creatingReport} style={{ minWidth: 400, height: 100 }} closable={false}>
        {!progress ? `${t('sts.txt.processing')}...` : <p className="m-0">{progress}</p>}
      </Dialog>
...
export default ViewLoadsCriteria;

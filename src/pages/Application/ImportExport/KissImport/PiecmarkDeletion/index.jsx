...
import {
  kissDeleteIdFiles,
  kissDeleteTransactions,
  kissImportReportsLoadsIdFiles,
} from 'api/api.kiss';
...
const PiecemarkDeletion = () => {
  ...
  useEffect(() => {
    refreshExpiration(kissImportReportsLoadsIdFiles, intervalID.current);
    return () => {
  ...
  const refreshExpiration = async (request, interval) => {
    try {
      const res = await request(id, { Limit: 0, Offset: 0 });
  ...
  const initTable = async (params = DEFAULT_LIMIT_LOAD_PARAMS, option) => {
    setLoadingTop(true);
    try {
      const res = await kissImportReportsLoadsIdFiles(id, params);
      setDataTop(res);
  ...
  const loadMoreTop = async (params) => {
    setLoadingTop(true);
    try {
      const res = await kissImportReportsLoadsIdFiles(id, {
        ...tableSettingsParams,
        ...params,
      });
...
export default PiecemarkDeletion;
